...
def analyze_clinical_case(payload: Dict[str, Any], file_name: str | None = None) -> Dict[str, Any]:
    ...
    if file_name:
        ...
        if lower.endswith((".png", ".jpg", ".jpeg", ".dcm")):
            score += 6
            findings.append("Imaging input processed through the federated scan analysis pipeline.")
        elif lower.endswith(".pdf"):
            findings.append("PDF report content registered for clinical pattern matching.")
        elif lower.endswith(".csv") or lower.endswith(".json"):
            findings.append("Structured report fields aligned with federated feature schema.")
