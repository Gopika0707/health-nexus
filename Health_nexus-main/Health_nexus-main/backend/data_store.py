...
class Patient(Base):
    ...
    if SQLALCHEMY_AVAILABLE:
        ...
        vitals_history = Column(JSON, default=list)
...
def _run_schema_updates() -> None:
    statements = [
        ...
        "ALTER TABLE patients ADD COLUMN vitals_history JSON NULL",
    ]
...
def _build_vitals_history(patient_public_id: str, latest: dict[str, Any], risk: Optional[str] = None) -> list[dict[str, Any]]:
    base = datetime.utcnow() - timedelta(days=6)
    status = "normal" if risk in {None, "Low", "Moderate"} else "warning"
    readings = []
        ...
        readings.append(
            {
                ...
                "timestamp": timestamp,
                "heart_rate": max(52, int(latest.get("heart_rate", 75) + (offset % 3) - 1)),
                "systolic_bp": max(90, int(latest.get("systolic_bp", 120) + ((offset % 4) - 1))),
                "diastolic_bp": max(60, int(latest.get("diastolic_bp", 80) + ((offset % 3) - 1))),
                "blood_sugar": max(70, int(latest.get("blood_sugar", 95) + ((offset % 5) - 2))),
                "spo2": max(90, int(latest.get("oxygen_level", 98) - (offset % 2))),
                "temperature": float(latest.get("temperature", 98.1 + (offset % 3) * 0.2)),
                "bmi": float(latest.get("bmi", 24.0 + (offset % 2) * 0.2)),
...
def _normalize_vitals_history(patient_public_id: str, latest: dict[str, Any], history: Optional[list[Any]], risk: Optional[str] = None) -> list[dict[str, Any]]:
    if history:
        ...
        for index, entry in enumerate(history):
            ...
            normalized.append(
                {
                    "id": str(entry.get("id") or f"VIT-{patient_public_id}-{index}"),
                    "patient_id": patient_public_id,
                    "timestamp": str(entry.get("timestamp") or _utc_now()),
                    "heart_rate": int(entry.get("heart_rate", latest.get("heart_rate", 75))),
                    "systolic_bp": int(entry.get("systolic_bp", latest.get("systolic_bp", 120))),
                    "diastolic_bp": int(entry.get("diastolic_bp", latest.get("diastolic_bp", 80))),
                    "blood_sugar": int(entry.get("blood_sugar", latest.get("blood_sugar", 95))),
                    "spo2": int(entry.get("spo2", latest.get("oxygen_level", 98))),
                    "temperature": float(entry.get("temperature", latest.get("temperature", 98.4))),
                    "bmi": float(entry.get("bmi", latest.get("bmi", 24.5))),
                    "respiratory_rate": int(entry.get("respiratory_rate", 16)),
                    "status": str(entry.get("status", "normal" if risk in {None, "Low", "Moderate"} else "warning")),
                }
            )
        normalized.sort(key=lambda item: item["timestamp"])
        return normalized
    return _build_vitals_history(patient_public_id, latest, risk)
...
def _serialize_patient(patient: Patient) -> dict[str, Any]:
    ...
    vitals_history = _normalize_vitals_history(patient.patient_id, latest_vitals, patient.vitals_history or None, patient.risk)
    return {
        "patient_id": patient.patient_id,
    ...
    try:
        if session.query(User).count() > 0:
            ...
            for demo in demo_patients:
                ...
                if not patient.vitals_history:
                    patient.vitals_history = _build_vitals_history(patient.patient_id, patient.latest_vitals or {}, patient.risk)
                    changed = True
        ...
        patients = [
            Patient(
                ...
                vitals_history=_build_vitals_history("PNX-84731", {"heart_rate": 82, "systolic_bp": 135, "diastolic_bp": 88, "blood_sugar": 108, "cholesterol": 195, "ecg": "normal sinus rhythm", "oxygen_level": 97}, "Moderate"),
            ...
            Patient(
                ...
                vitals_history=_build_vitals_history("PNX-29183", {"heart_rate": 96, "systolic_bp": 154, "diastolic_bp": 96, "blood_sugar": 186, "cholesterol": 242, "ecg": "st depression", "oxygen_level": 95}, "High"),
            ...
            Patient(
                ...
                vitals_history=_build_vitals_history("PNX-57234", {"heart_rate": 108, "systolic_bp": 128, "diastolic_bp": 84, "blood_sugar": 102, "cholesterol": 201, "ecg": "irregular qrs", "oxygen_level": 96}, "Moderate"),
            ...
            Patient(
                ...
                vitals_history=_build_vitals_history("PNX-83920", {"heart_rate": 88, "systolic_bp": 118, "diastolic_bp": 76, "blood_sugar": 92, "cholesterol": 168, "ecg": "normal", "oxygen_level": 99}, "Low"),
...
def get_patient_vitals(patient_public_id: str) -> list[dict[str, Any]]:
    ...
    history = patient.get("vitals_history") or []
    if history:
        return _normalize_vitals_history(patient_public_id, latest, history, patient.get("risk"))
    return _build_vitals_history(patient_public_id, latest, patient.get("risk"))
...
def update_patient_vitals(patient_public_id: str, vitals: dict[str, Any]) -> dict[str, Any]:
    ensure_store()
    session = _get_session()
    try:
        ...
        existing = dict(patient.latest_vitals or {})
        history = list(patient.vitals_history or [])

        def _int_value(value: Any, fallback: Any) -> int:
            candidate = value if value is not None else fallback
            return int(candidate or 0)

        def _float_value(value: Any, fallback: Any) -> float:
            candidate = value if value is not None else fallback
            return float(candidate or 0.0)

        updated = {
            "heart_rate": _int_value(vitals.get("heart_rate"), existing.get("heart_rate")),
            "systolic_bp": _int_value(vitals.get("systolic_bp"), existing.get("systolic_bp")),
            "diastolic_bp": _int_value(vitals.get("diastolic_bp"), existing.get("diastolic_bp")),
            "blood_sugar": _int_value(vitals.get("blood_sugar"), existing.get("blood_sugar")),
            "cholesterol": _int_value(existing.get("cholesterol"), 0),
            "ecg": str(existing.get("ecg", "")),
            "oxygen_level": _int_value(existing.get("oxygen_level"), 0),
            "temperature": _float_value(vitals.get("temperature"), existing.get("temperature", 98.4)),
            "bmi": _float_value(vitals.get("bmi"), existing.get("bmi", 24.5)),
        }

        if not history:
            history = _build_vitals_history(patient_public_id, existing, patient.risk)

        history.append(
            {
                "id": f"VIT-{patient_public_id}-{len(history)}",
                "patient_id": patient_public_id,
                "timestamp": _utc_now(),
                "heart_rate": updated["heart_rate"],
                "systolic_bp": updated["systolic_bp"],
                "diastolic_bp": updated["diastolic_bp"],
                "blood_sugar": updated["blood_sugar"],
                "spo2": updated["oxygen_level"],
        ...
        patient.latest_vitals = updated
        patient.vitals_history = history
        patient.last_visit = _utc_now().split("T", 1)[0]
        session.commit()
...
def delete_patient_vitals(patient_public_id: str) -> dict[str, Any]:
    ensure_store()
    session = _get_session()
    try:
        patient = session.query(Patient).filter_by(patient_id=patient_public_id).first()
        ...
        history = list(patient.vitals_history or [])
        if len(history) <= 1:
            raise ValueError("At least one vitals entry must remain.")

        history.pop()
        latest = history[-1]
        patient.latest_vitals = {
            "heart_rate": int(latest.get("heart_rate", 0)),
            "systolic_bp": int(latest.get("systolic_bp", 0)),
            "diastolic_bp": int(latest.get("diastolic_bp", 0)),
            "blood_sugar": int(latest.get("blood_sugar", 0)),
            "cholesterol": int(patient.latest_vitals.get("cholesterol", 0) if patient.latest_vitals else 0),
            "ecg": str(patient.latest_vitals.get("ecg", "") if patient.latest_vitals else ""),
            "oxygen_level": int(latest.get("spo2", latest.get("oxygen_level", 0))),
            "temperature": float(latest.get("temperature", 98.4)),
            "bmi": float(latest.get("bmi", 24.5)),
        }
        patient.vitals_history = history
        patient.last_visit = str(latest.get("timestamp", _utc_now())).split("T", 1)[0]
        session.commit()
        session.refresh(patient)
        return _serialize_patient(patient)
    finally:
...
def get_patient_mental_health(patient_public_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "recommendations": [
            ...
            "Take short stress-management breaks during work.",
            "Continue routine follow-up if symptoms worsen.",
        ],
    }
