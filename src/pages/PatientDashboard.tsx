...
export default function PatientDashboard() {
  ...
  const [liveConnectionState, setLiveConnectionState] = useState<"connecting" | "connected" | "disconnected">("disconnected");
  ...
  // Define load function as a callback so it can be used in buttons and effects
  const loadPatientData = useCallback(async (silent = false) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (!cancelled) await loadPatientData(false);
    })();
    
    // Set up polling intervals:
    // - 10 seconds if EventSource is disconnected (fallback)
    // - 60 seconds if EventSource is connected
    let pollInterval: number | null = null;
    let eventSourceConnectAttempts = 0;
    ...
    // Start with fallback polling, EventSource will stop it if successful
    ...
    const setupEventSource = () => {
      ...
      try {
        source = new EventSource(API_ENDPOINTS.patientEvents(patientId, token));

        source.addEventListener("ready", () => {
          if (!cancelled) {
            setLiveConnectionState("connected");
            isEventSourceConnected = true;
            stopFallbackPolling(); // Stop polling when EventSource is connected
            eventSourceConnectAttempts = 0;
        ...
        source.addEventListener("patient-update", refreshFromLiveEvent);
        source.addEventListener("message", refreshFromLiveEvent);
        ...
        source.addEventListener("error", () => {
          if (!cancelled) {
            ...
            eventSourceConnectAttempts++;
            if (eventSourceConnectAttempts < 5) {
              if (connectionTimeoutId) clearTimeout(connectionTimeoutId);
              connectionTimeoutId = setTimeout(setupEventSource, 5000);
            }
          }
        });

        // Connection timeout: if not ready after 10 seconds, try fallback
        if (connectionTimeoutId) clearTimeout(connectionTimeoutId);
        connectionTimeoutId = setTimeout(() => {
          if (source && !isEventSourceConnected && !cancelled) {
            console.warn("EventSource connection timeout, starting fallback polling");
            source.close();
            startFallbackPolling();
          }
        }, 10000);
  ...
  }, [isAuthenticated, navigate, patientId, role, loadPatientData]);
  ...
  const liveDoctorSummary = patient?.recommendation ?? selectedReport?.ai_summary ?? "Low immediate risk. Continue preventive monitoring and routine follow-up.";
    ...
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="rounded-3xl border border-sky-200 bg-sky-50/80 p-4 text-slate-700 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="text-xs font-semibold uppercase tracking-wide text-sky-700">Live Tracking</div>
                <div className="mt-1 text-sm">Patient data refreshes instantly from the doctor portal, with a fallback refresh every 10 seconds if real-time sync is unavailable.</div>
              </div>
              <div className="text-right text-xs space-y-1">
                <div className="rounded-full bg-white px-3 py-1 font-semibold text-sky-700">
                  {liveConnectionState === "connected" ? "Live push connected" : liveConnectionState === "connecting" ? "Connecting live sync..." : "Live sync reconnecting"}
                </div>
                <div className="uppercase tracking-wide text-slate-500">Last synced</div>
                <div className="font-semibold text-slate-900">{lastSyncedAt ? new Date(lastSyncedAt).toLocaleString() : "Waiting for first sync..."}</div>
                <Button 
                  variant="outline" 
                  size="sm"
                  onClick={() => void loadPatientData(false)}
                  disabled={isLoading}
                  title="Manually refresh patient data"
          ...
          <div className="grid grid-cols-2 gap-4 xl:grid-cols-6">
            {[
              { label: "Heart Rate", value: latestVital?.heart_rate ?? "-", unit: "bpm", icon: Heart },
              { label: "Blood Pressure", value: `${patient.latest_vitals.systolic_bp}/${patient.latest_vitals.diastolic_bp}`, unit: "mmHg", icon: Activity },
              { label: "Blood Sugar", value: latestVital?.blood_sugar ?? "-", unit: "mg/dL", icon: Sparkles },
              { label: "SpO2", value: latestVital?.spo2 ?? "-", unit: "%", icon: Wind },
              { label: "Temperature", value: latestVital?.temperature ?? "98.4", unit: "°F", icon: Thermometer },
              { label: "BMI", value: latestVital?.bmi ?? "24.5", unit: "kg/m²", icon: TrendingUp },
            ].map((item) => (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="rounded-3xl border border-slate-200 bg-white p-4">
                  <div className="text-xs uppercase tracking-wide text-slate-500">Doctor Sync</div>
                  <div className="mt-2 text-sm font-semibold text-slate-900">{patient.last_visit ?? "Not updated yet"}</div>
                  <div className="text-sm text-slate-600">Last visit timestamp from the doctor portal.</div>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-white p-4">
                  <div className="text-xs uppercase tracking-wide text-slate-500">Current Condition</div>
                  <div className="mt-2 text-sm font-semibold text-slate-900">{patient.condition ?? "General Monitoring"}</div>
                  <div className="text-sm text-slate-600">This reflects the latest doctor-side update.</div>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-white p-4">
                  <div className="text-xs uppercase tracking-wide text-slate-500">Current Risk</div>
                  <div className="mt-2 text-sm font-semibold text-slate-900">{patient.risk ?? "Low"}</div>
                  <div className="text-sm text-slate-600">Changes here come from the doctor portal live sync.</div>
                </div>
              </div>
              <div>
                <h2 className="font-display text-4xl font-bold text-slate-900">Vital Monitoring Engine</h2>
                <p className="mt-2 text-lg text-slate-500">AI-powered trend analysis with 7-day forecast</p>
              </div>
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                    Follow-up: {selectedReport?.follow_up ?? patient.recommendation ?? "Continue routine monitoring."}
                  </div>
                  {nextForecast ? (
                    <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                ...
                {mentalHealth ? (
                  <div className="mt-8 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                    ...
                    {mentalHealth.recommendations?.length ? (
                      <div className="mt-4 space-y-2">
                        {mentalHealth.recommendations.map((recommendation, idx) => (
                          <div key={idx} className="rounded-xl bg-white p-3 text-slate-600">
                            <div className="text-xs font-semibold uppercase tracking-wide text-sky-700">{recommendation.category}</div>
